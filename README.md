...
- **Dictaphone** — voice recording via glasses mic
- **Scene narrator** — real-time camera description
...
| `ui/StreamScreen.kt` | Main screen: Gemini + Twitch + mic picker |
...
DAT SDK video stream (24fps, I420)
   |
   |--- raw I420 --> DirectRtmpEncoder --> H.264 --> RTMP --> Twitch
   |
   | (only when screen is on OR Gemini is active)
